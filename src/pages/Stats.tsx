...
import type { Exercise, SetRecord, Workout } from '../db/types'
  ...
  const computed = useMemo(() => {
    ...
    const prs = [...prMap.entries()]
      ...
      .map(({ ex, set }) => {
        ...
        let val = ''
        if (metric === 'weight_reps') val = `${set.weight || 0}kg × ${set.reps || 0}`
        else if (metric === 'reps') val = `${set.reps || 0} 次`
        else if (metric === 'time') val = `${set.durationSec || 0} 秒`
        else val = `${set.distance || 0} km`
        return { name: ex!.name, category: ex!.category, metric, val }
      })
      .sort((a, b) => {
        const order = ['胸', '背', '腿', '肩', '手臂', '核心', '有氧', '其他']
  ...
  return (
    <Page title="统计">
      ...
      <div className="card">
        <h3 className="card-title">
          训练容量趋势
          <span className="small muted" style={{ fontWeight: 400 }}>近 12 周 · 单位 kg·次</span>
        </h3>
        <VolumeBar buckets={computed.volBuckets} />
      </div>
      ...
      <div className="card">
        ...
        <CategoryPie data={computed.cats} />
      </div>
      ...
      <div className="card">
                ...
                {col.map(c => (
                  <div
                    ...
                    style={{
                      ...
                      background: c.inFuture ? 'transparent' : c.trained ? 'var(--brand)' : '#eceef1',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, justifyContent: 'flex-end' }}>
          ...
          <span style={{ width: 14, height: 14, borderRadius: 4, background: '#eceef1' }} />
...
// ===== WeeklyBar（P2） =====
function WeeklyBar({ data, avg }: { data: WeekStat[]; avg: number }) {
    ...
    chart.setOption({
      ...
      series: [{
        ...
        markLine: avg > 0 ? {
          ...
          label: { formatter: `均 ${avg}`, fontSize: 10, color: '#9ca3af' },
          data: [{ yAxis: avg }],
        } : undefined,
      }],
    })
...
// ===== VolumeBar（P5 新增） =====
function VolumeBar({ buckets }: { buckets: { label: string; volume: number }[] }) {
  const ref = useRef<HTMLDivElement>(null)
  useEffect(() => {
    if (!ref.current) return
    const chart = echarts.init(ref.current)
    chart.setOption({
      ...
      tooltip: {
        trigger: 'axis',
        formatter: (p: any) => `${p[0].name}<br/>${(p[0].value || 0).toLocaleString()} kg·次`,
      },
      xAxis: {
        type: 'category',
...
// ===== CategoryPie（P5 新增） =====
function CategoryPie({ data }: { data: { name: string; value: number; color: string }[] }) {
  const ref = useRef<HTMLDivElement>(null)
  useEffect(() => {
    if (!ref.current) return
    const chart = echarts.init(ref.current)
    chart.setOption({
      tooltip: {
        trigger: 'item',
        formatter: (p: any) => `${p.name}<br/>${p.value.toLocaleString()} kg·次 (${p.percent}%)`,
      },
      legend: {
